\subsection*{Lecture}
This weeks lecture dealt with clustering data. 
...
However, the result is strongly dependent on input order, highly sensitive to the chosen threshold and tends to loose the global view of the data as only local similarity is used in the metric. 
It is also prone for chaining together neaarby data points and bridge different clusters. 
\todo{anything else?}\\
...
The algorithm is however not well suited for data that is not spherical, struggels with overlapping groups and outliers and is sensitive to initialization. 
...
Validation can be done externally (if known labels are present), internally (measure compactness statistically) and relatively (compare clusterings obtained by the same algorithm for different parameters). 
...
\subsection*{Exercise}
This weeks exercise was a small hands on example of the workflow behind clustering data based on the \lstinline{toolo.csv} dataset. 
...
After this k-mean clustering with 7 clusters is employed to compare Sammon maps, boxplots and Silhouette plots graphically. 
...
Although the difference to the other normalization idices is quite small (roughly in the range of 0.1 for both) it is already a good indication, that Euclidean normalization might be best suited. 
...
The Silhouette plot also suggests that there may be some overlap between clusters 5 and 6, as they are less clearly separated than the other clusters. 
...
\todo{fig}
As can be seen, 2 clusters seems to work best. 
...
\todo{fig}
As can be seen, the 2 clusters can be easily understood. 
...
From the analysis of just two clusters not a whole lot of insights can be gained. 
